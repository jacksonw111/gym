...
export const bookLesson = async (store: Store, input: BookLessonInput): Promise<Lesson> =>
  store.transaction(() => {
    ...
    if (occupied) throw new Error('该时段已被预约')
    const membership = store.packages.find((item) => item.id === input.packageId)
    if (!membership || membership.memberId !== input.memberId) throw new Error('课包不存在')
    if (membership.coachId !== input.coachId) throw new Error('课包与教练不匹配')
    if (membership.availableLessons < 1) throw new Error('可用课时不足')
...
const getBookedLessonAndPackage = (store: Store, lessonId: string) => {
  ...
  if (lesson.status !== 'booked') throw new Error('终态课程不能再次转换')
...
export const completeLesson = async (store: Store, input: CompleteLessonInput): Promise<Lesson> =>
  store.transaction(() => {
    ...
    if (input.actor.kind === 'admin') throw new Error('管理员不能完成课程')
    ...
    if (input.actor.kind === 'system') {
      if (elapsed < 24 * 60 * 60 * 1000) throw new Error('结束未满24小时')
    } else if (elapsed < 0) {
      throw new Error('课程尚未结束')
    }
...
export const saveFeedback = async (
  ...
  store.transaction(() => {
    const lesson = store.lessons.find((item) => item.id === lessonId)
    if (!lesson || lesson.memberId !== memberId) throw new Error('课程不存在或无权限')
    if (lesson.status !== 'completed') throw new Error('只能评价已完成课程')
    if (lesson.feedback) throw new Error('反馈已经提交')
    if (
      feedback.rating !== undefined &&
      (!Number.isInteger(feedback.rating) || feedback.rating < 1 || feedback.rating > 5)
    ) {
      throw new Error('星级必须为1到5')
    }
